...
import focusRelevantRules from './focus-relevant.rules';
...
export default function rules(element, except = {
  flexbox: false,
  scrollable: false,
  shadow: false,
}) {
  ...
  if (platform.is.TRIDENT) {
    ...
    if (!except.flexbox && hasCssDisplayFlex(style)) {
      if (_tabindex === null) {
        const _except = { flexbox: true };
        return focusRelevantRules(element, _except) && rules(element, _except);
      }
